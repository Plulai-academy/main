...
export default function DashboardClient({
  ...
  return (
    <div className="p-6 lg:p-10 max-w-5xl">
      {/* Level-up modal */}
      {levelUpMsg && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setLevelUpMsg(null)}>
          <div className="bg-card border border-accent2/30 rounded-3xl p-10 text-center shadow-2xl animate-star-pop">
            <div className="text-6xl mb-4 animate-bounce-slow">🎊</div>
            <h2 className="font-fredoka text-3xl text-accent2 mb-2">{levelUpMsg}</h2>
      ...
      {/* Toast */}
      <div className={cn('fixed top-6 right-6 z-50 px-5 py-3 rounded-2xl bg-card border border-accent3/30 text-accent3 font-bold text-sm shadow-xl transition-all duration-400', toast ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-3 pointer-events-none')}>
        {toast}
      </div>

      {/* Greeting */}
      <div className="mb-8 animate-slide-up">
        <h1 className="font-fredoka text-3xl lg:text-4xl mb-1">{greetings[profile.age_group] || greetings.pro}</h1>
      ...
      {/* XP Card */}
      <div className="bg-card rounded-3xl p-6 border border-white/5 mb-6 shadow-xl animate-slide-up">
        <div className="flex items-start justify-between mb-4">
          <div>
            <div className="flex items-center gap-2 mb-0.5">
              <span className="font-fredoka text-2xl text-accent2">Level {level}</span>
              <span className="text-accent2/60 font-bold text-sm">— {getLevelTitle(level)}</span>
            </div>
            <div className="text-muted text-sm font-bold">{xpInLevel} / {XP_PER_LEVEL} XP to next level</div>
          </div>
          <div className="text-right">
            <div className="font-fredoka text-3xl text-white">{formatXP(xp)}</div>
            <div className="text-muted text-xs font-bold">Total XP</div>
          </div>
        </div>
        <div className="h-5 bg-card2 rounded-full overflow-hidden mb-4">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            <div key={s.label} className="bg-card2 rounded-2xl p-3 text-center">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Daily Challenge */}
        <div className="bg-card rounded-3xl p-6 border border-white/5 shadow-xl animate-slide-up">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-fredoka text-xl">⚡ Today&apos;s Challenge</h2>
            {todayChallenge && <span className="text-xs font-bold bg-accent2/15 text-accent2 border border-accent2/25 px-3 py-1 rounded-full">+{todayChallenge.xp_reward} XP</span>}
          </div>
          {todayChallenge ? (
            <>
              <div className="bg-card2 rounded-2xl p-5 mb-4">
                <div className="text-3xl mb-2">{todayChallenge.emoji}</div>
                <h3 className="font-extrabold text-base mb-1">{todayChallenge.title}</h3>
              ...
              <button
                ...
                className={cn('w-full py-3.5 rounded-2xl font-extrabold text-sm transition-all',
                  isChallengeComplete
            ...
            <div className="text-center py-8 text-muted font-bold">No challenge today — check back tomorrow! 🌅</div>
          )}
        </div>
        ...
        <div className="bg-card rounded-3xl p-6 border border-white/5 shadow-xl animate-slide-up">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-fredoka text-xl">🏆 Badges</h2>
          ...
          {earnedBadges.length > 0 ? (
            <div className="grid grid-cols-3 gap-3 mb-4">
              {earnedBadges.slice(0,6).map((b: any) => (
                <div key={b.id} className={cn('bg-card2 rounded-2xl p-3 text-center border hover:scale-105 transition-all cursor-default', getRarityColor(b.rarity), getRarityGlow(b.rarity))}>
              ...
              {lockedBadges.map((b: any) => (
                <div key={b.id} className="flex items-center gap-3 bg-card2 rounded-xl px-3 py-2.5 mb-2 opacity-60">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-muted">{b.condition}</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* Dream Project */}
        <div className="lg:col-span-2 bg-gradient-to-br from-accent5/10 to-accent4/10 rounded-3xl p-6 border border-accent5/20 shadow-xl animate-slide-up">
          ...
          <div className="flex flex-wrap gap-3">
            <Link href="/dashboard/coach" className="inline-block px-6 py-3 rounded-2xl font-extrabold text-sm bg-gradient-to-r from-accent5 to-accent4 text-white hover:-translate-y-1 transition-all">
              🤖 Talk to your AI Coach about it →
            </Link>
            <button
              ...
              className="inline-block px-6 py-3 rounded-2xl font-extrabold text-sm border border-accent2/30 text-accent2 hover:bg-accent2/10 hover:-translate-y-1 transition-all"
            >
              📤 Share my progress
            </button>
          </div>
