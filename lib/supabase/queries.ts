...
import { awardStreakCoins, awardXPCoins } from './wallet'
...
export const addXP = async (userId: string, amount: number, reason: string, sourceId?: string) => {
  ...
  // Calls wallet function directly — no HTTP round-trip needed.
  // Non-blocking — coin failures never affect XP.
  try {
    await awardXPCoins(userId, xpBefore, xpAfter)
  } catch (e) {
    console.error('[addXP] coin award failed:', e)
  }
  // ─────────────────────────────────────────────────────────────
...
export const updateStreak = async (userId: string) => {
  ...
  if (streakIncremented) {
    try {
      ...
      console.error('[updateStreak] coin award failed:', e)
    }
  }
  // ─────────────────────────────────────────────────────────────
