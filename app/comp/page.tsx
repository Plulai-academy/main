...
// ── Countdown hook (fixed: no missing dependency) ─────────────────────────
function useCountdown(targetTimestamp: number) {
  ...
  }, [targetTimestamp]); // ✅ dependency added
  return t;
}

// ── Data ────────────────────────────────────────────────────────────────────
const DEADLINE = new Date("2025-06-15T23:59:59");
const DEADLINE_TIMESTAMP = DEADLINE.getTime(); // ✅ stable value for hook
...
// ── Registration Form (with fixed apostrophes) ───────────────────────────────
...
function RegisterForm() {
  ...
  if (status === "success") return (
    <div style={{
      ...
      <div style={{ fontWeight: 800, fontSize: "1.4rem", color: "#fff", marginBottom: 10 }}>You&apos;re registered!</div>            {/* ✅ fixed apostrophe */}
  ...
  return (
    <form onSubmit={submit} style={{
      ...
      <div>
        <label style={labelStyle}>Child&apos;s Full Name · اسم الطفل</label>          {/* ✅ fixed apostrophe */}
      ...
      <div>
        <label style={labelStyle}>Child&apos;s Age · عمر الطفل</label>              {/* ✅ fixed apostrophe */}
      ...
      >
        {status === "loading" ? "⏳ Registering..." : "🎯 Register & Pay 10 DT · سجّل الآن"}
      </button>

      <div style={{ fontSize: "0.75rem", color: "#6b7280", textAlign: "center", lineHeight: 1.6 }}>
        By registering you confirm your child is aged 6–18 and agrees to Plulai&apos;s Terms.     {/* ✅ fixed apostrophe */}
...
export default function PlulaiTunisiaPage() {
  const { days, hours, mins, secs } = useCountdown(DEADLINE_TIMESTAMP); // ✅ uses stable timestamp
  ...
  const css = `
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    html, body { background: #05050d; scroll-behavior: smooth; }  /* ✅ added body background */
    body { margin: 0; }
    @keyframes fadeUp {
        ...
        <section style={{ padding: "0 20px 100px", maxWidth: 800, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 14 }}>
            {[
              ...
              { icon: "🎯", title: "Real Skills", ar: "مهارات حقيقية", desc: "Python, AI concepts, logical thinking. Skills that schools don&apos;t yet teach — but employers demand." }, // ✅ fixed
